...
class RatingTransformer extends BaseTransformer
{
    ...
    public function transform(Rating $rating)
    {
        ...
        $transformedArray = [
            ...
            'title' => (strlen($rating->title) <= 30) ? $rating->title : \Str::limit($rating->title, 30,
                generatePopover($rating->title)),
            ...
            'status' => formatStatusAsLabels($status,
                ['level' => $levels[$status], 'text' => trans('Utility::attributes.rating.status_options.' . $status)]),
