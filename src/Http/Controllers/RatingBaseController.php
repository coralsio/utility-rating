...
class RatingBaseController extends BaseController
{
    ...
    public function __construct(RatingService $ratingService)
    {
        ...
        $this->resource_url = config('utility-rating.models.rating.resource_url');

        $this->title = trans('utility-rating::module.rating.title');
        $this->title_singular = trans('utility-rating::module.rating.title_singular');
                ...
                case 'pending':
                    foreach ($selection as $selection_id) {
                        ...
                        if (user()->can('Utility::rating.update')) {
                            ...
                            $message = [
                                'level' => 'success',
                                'message' => trans('utility-rating::attributes.update_status',
                                    ['item' => $this->title_singular])
                            ];
                        } else {
                            $message = [
                                'level' => 'error',
                                'message' => trans('utility-rating::attributes.no_permission',
                                    ['item' => $this->title_singular])
                            ];
                        }
                    }
